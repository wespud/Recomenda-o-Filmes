# app.py — Protótipo de Feed de Recomendações 
# TCC: Algoritmos de Recomendação
...
# TMDB
# Configurações → API
TMDB_API_KEY  = "8441c5ad2f7481c2c1ebe7599089e037"
...
# Lista de todos os gêneros em PT 
...
def exibir_perfil(selected_user, users_df, ratings_df, movies_df):
    ...
    Também exibe watchlist e avaliações feitas na sessão.
    """
...
def buscar_poster(titulo: str):
    ...
    Retorna vazio se a chave não estiver configurada ou o filme não for encontrado.
    """
    if not TMDB_API_KEY:
        return None
...
def diversity_at_k(rec, movies_df, k=10):
    ...
    try:
        ...
        pairs = n * (n - 1) / 2
        return max(0.0, total / pairs) if pairs > 0 else 0.0
    except Exception:
        return 0.0
...
# ══════════════════════════════════════════════════════════════
# CABEÇALHO COLORIDO POR ALGORITMO (banner SVD / TF-IDF)
# ══════════════════════════════════════════════════════════════

def render_algo_header(tipo: str):
    if tipo == 'svd':
        cor       = '#8aa9ff'
        bg        = 'rgba(138, 169, 255, 0.08)'
        border    = 'rgba(138, 169, 255, 0.3)'
        sigla     = 'SVD'
        nome      = 'Filtragem Colaborativa'
        descricao = 'Baseada no comportamento de usuários similares'
    else:
        cor       = '#c896f0'
        bg        = 'rgba(200, 150, 240, 0.08)'
        border    = 'rgba(200, 150, 240, 0.3)'
        sigla     = 'TF-IDF'
        nome      = 'Baseada em Conteúdo'
        descricao = 'Baseada nos gêneros do histórico do usuário'
...
# ══════════════════════════════════════════════════════════════
# CARD DE FILME COM POSTER, GÊNEROS, AVALIAÇÃO E BOTÕES DE INTERAÇÃO
# ══════════════════════════════════════════════════════════════
...
def render_movie_card(i: int, row, selected_user: int, ratings_df):
    ...
    - Botão 👍 Curtir com popover de nota
    - Botão 🔖 Salvar com lógica de watchlist
    """
...
# ══════════════════════════════════════════════════════════════
# PAINEL DE NOVO USUÁRIO (sidebar)
# ══════════════════════════════════════════════════════════════
...
def painel_novo_usuario(ratings_df, movies_df):
    ...
    with st.sidebar.expander("➕ Criar novo usuário", expanded=False):
        ...
        if enviado:
            ...
            novos_existentes = st.session_state['novos_usuarios']
            novo_id = max(
                int(ratings_df['user_id'].max()),
                max(novos_existentes.keys(), default=0)
            ) + 1
...
st.caption("TCC — ANÁLISE E FUNCIONAMENTO DE ALGORITMOS DE RECOMENDAÇÃO EM PLATAFORMAS DE STREAMING E REDES SOCIAIS")
...
with col_feed:
    ...
    if rec_mode is not None:
        if "Comparar" in rec_mode:
            ...
            cc, cb = st.columns(2)
            with cc:
                render_algo_header('svd')
                feed_c = _feed(collab_df, selected_user)
                if feed_c is not None:
                    for i, row in enumerate(feed_c.itertuples()):
                        render_movie_card(i, row, selected_user, ratings_ativos)
            with cb:
                render_algo_header('content')
                feed_b = _feed(content_df, selected_user)
                if feed_b is not None:
                    for i, row in enumerate(feed_b.itertuples()):
                        render_movie_card(i, row, selected_user, ratings_ativos)

        else:
            st.subheader(f"📱 Feed — Usuário {selected_user}")
